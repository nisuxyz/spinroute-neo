...
├── lib/
│   ├── auth.ts              # Service-specific auth
│   ├── auth-middleware.ts
│   ├── config.ts
│   ├── db.ts or drizzle.ts  # Database client
...
- **Drizzle**: Used for backend and some services (`drizzle/` folder)
- **Supabase**: Used for services with Supabase schemas (`supabase/migrations/` folder)
- Migration files are timestamped SQL or generated by Drizzle Kit
